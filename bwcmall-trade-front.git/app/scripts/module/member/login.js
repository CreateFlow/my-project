        ...
        var loginCtrl = avalon.define('loginCtrl', function(vm) {
            ...
            vm.checkPhone = function() {
                ...
                if (userName.replace(/\s+/g, '') === '') {
                    vm.setErrorInfo(true, '请输入手机号！');
                } else if (pattern.test(userName) === false) {
                    vm.setErrorInfo(true, '请输入正确的手机号');
                } else {
                ...
                if (vm.valid === true && vm.validate() === true) {
                    ...
                    common.login(vm.$model.user, '.jSubmit', function(data) {
                        ...
                        if (data.success === true) {
                            // 增加对上传三证用户对判断
                            // 未完成上传三证用户跳至转跳页面
                            if ('licensePassed' in result && result.licensePassed === false) {
                                window.location.href = urlConfig.regIntro;
                            } else {
            ...
            vm.forgetPassword = function(){
                var url = "/member/findPassword.html";
                //var hostname = urls.get('hostname');

                if(vm.user.userName != ''){
                    url += "?username=" + vm.user.userName;
                }
